...
## 🏗️ 영지 생산 건물 (Production Buildings)
영지에 건설하여 매 턴(라운드)마다 자동으로 자원을 획득할 수 있는 시설입니다.

- **🏦 은행**: 턴마다 **골드(Gold)** 생산.
- **🏭 공장**: 턴마다 **벽돌(Brick)** 생산. (건물 건설 및 업그레이드 재료)
- **⛪ 성당**: 턴마다 **전능의 정수(Divine Essence) ✨** 생산. (메시아의 권능 강화 자원)
- **🏕️ 캠프**: 턴마다 **몬스터 고기(Monster Meat) 🍖** 생산. (펫 강화 및 성장 자원)
- **🌳 나무**: 턴마다 **나무(Wood)** 생산. (기초 건설 재료)
- **🏰 성**: 턴마다 **다이아(Diamond) 💎** 생산. (매우 긴 생산 주기를 가짐)
