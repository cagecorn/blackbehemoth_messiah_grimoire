...
| `bao` | 바오 | `wizard` | `SkillStoneBlast` | 곰 수인 마법사. 동생 바바오와의 유대가 깊은 마법사. |
...
- **메테오 스트라이크 (Meteor Strike)**
  - **사용자**: 멀린 (`merlin`)
  - **효과**: 화면 전체를 뒤덮는 운석 비를 소환합니다. 50발의 운석이 무작위 위치에 떨어져 강력한 광역 피해를 입힙니다.
- **메시아를 위하여! (Messiah, for You!)**
  - **사용자**: 아렌 (`aren`)
  - **효과**: 하늘로 높이 솟구친 뒤 가장 적이 밀집된 지역을 강타합니다. 범위 내 적들에게 4배의 피해를 입히고 에어본 후 기절(Stun) 상태로 만듭니다.
- **소환 : 수호 천사 (Summon: Guardian Angel)**
  - **사용자**: 세라 (`sera`)
  - **효과**: 전장을 지원하는 수호 천사를 소환합니다. 아군을 치유하고 적을 공격하며, 게이지를 다시 채워 사용 시 최대 3단계까지 강화됩니다.
- **소환 : 사이렌 (Summon: Siren)**
  - **사용자**: 루트 (`lute`)
  - **효과**: 매혹적인 선율의 사이렌을 소환합니다. 2단계 강화부터 음표 공격 시 적에게 **수면**을 부여하며, 3단계에서는 수면 확률이 50%까지 상승합니다.
- **왕년엔 말이야... (Back in my day...)**
  - **사용자**: 니클 (`nickle`)
  - **효과**: 니클이 자신의 리즈 시절로 변신합니다. 능력치가 폭등하고 한 번에 5발의 화살을 **순차적으로(다다다다!)** 발사합니다. 레이드 보스전에서는 보스에 근접하여 모든 화살을 꽂아 넣는 전용 AI가 작동합니다.
